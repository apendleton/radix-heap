...
template<typename T>
class radix_max_heap_test_all_types : public testing::Test {};
TYPED_TEST_CASE(radix_max_heap_test_all_types, AllTypes);

TYPED_TEST(encoder_test_all_types, extreme) {
  TypeParam xs[] = {0, numeric_limits<TypeParam>::lowest(), numeric_limits<TypeParam>::max()};
  for (TypeParam x : xs) {
    TypeParam y = radix_max_heap::internal::encoder<TypeParam>::decode
        (radix_max_heap::internal::encoder<TypeParam>::encode(x));
    ASSERT_EQ(x, y);
  }
}
...
TYPED_TEST(radix_max_heap_test_all_types, trivial) {
  radix_max_heap::radix_max_heap<TypeParam> h;
  ASSERT_TRUE(h.empty());
  ASSERT_EQ(0, h.size());
...
TYPED_TEST(radix_max_heap_test_all_types, extreme1) {
  radix_max_heap::radix_max_heap<TypeParam> h;
  h.push(0);
...
TYPED_TEST(radix_max_heap_test_all_types, extreme2) {
  radix_max_heap::radix_max_heap<TypeParam> h;
...
TYPED_TEST(radix_max_heap_test_all_types, copy_and_swap) {
  typedef radix_max_heap::radix_max_heap<TypeParam> heap_type;
...
TEST(pair_radix_max_heap_test, trivial) {
  radix_max_heap::pair_radix_max_heap<double, string> h;
...
TEST(pair_radix_max_heap_test_double_string, emplace) {
  radix_max_heap::pair_radix_max_heap<double, string> h;
  h.emplace(20, "hoge");
  h.emplace(10, 10, 'a');
  ASSERT_EQ("hoge", h.top_value());
...
TEST(pair_radix_max_heap_test_int_string, large) {
  const int kNumTrials = 10;
  const int kNumPop = 10000;
  const int kMaxDiff = 1000;
  const int kMaxInsert = 10;

  radix_max_heap::pair_radix_max_heap<int, string> rh;
  for (int trial = 0; trial < kNumTrials; ++trial) {
    rh.clear();
    ASSERT_TRUE(rh.empty());
    ...
    for (int i = 0; i < kNumPop; ++i) {
      ...
      ASSERT_EQ(top_key, (*se.rbegin()).first);
